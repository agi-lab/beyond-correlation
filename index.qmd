...
```{r, read_libraries, echo = FALSE, output = FALSE}
...
# png("cross_pair_multiline_rank.png", width = 900, height = 900, res = 450)
...
plot(rank(multi_line_dat$X2), rank(multi_line_dat$X3), pch = 16, cex = 0.35,
     xlab = "Rank of X2", ylab = "Rank of X1",
...
p <- ggplot(plot_df, aes(x = s)) +
  ...
  scale_colour_manual(
    ...
    labels = c(
      "joint"   = TeX(r'($\varphi_{XY}(s,1)$)'),
      "product" = TeX(r'($\varphi_X(s)\phi_Y(1)$)')
    ),
    name = NULL
  ) +
  labs(
...
The illustrations of the joint distance covariance involves model training and was done in `Python`. Refer to [this page](jdcov.qmd) for the specific code and results.
